...
from scraper.parsers.base_parser import logger, UnableToGetNProductUrls
...
class RozetkaParser(BaseParser):
    ...
    def __init__(self):
        ...
        website_navigation=WebsiteNavigationXPaths(search_field='/html/body/rz-app-root/div/div[1]/rz-main-header/header/div/div/div/rz-search-suggest/form/div[1]/div/div/input',
    ...
    def find_n_products(self, product, n, fast_parse=True, raise_exception=False, ignore_price_format=True):
        ...
        try:
            ...
            if raise_exception or len(result) == 0:
                raise UnableToGetNProductUrls(f'Unable to find products ({len(result)} found)') from e
